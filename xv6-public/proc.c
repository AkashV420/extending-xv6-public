...
int kataar[5][3000];
int aage[5] = {0,0,0,0,0};
int piche[5] = {0,0,0,0,0};
...
void push_back(int val, int i) {
  kataar[i][piche[i]] = val;   
  piche[i] = (piche[i]+1) % 10000;
}

void pop_front(int i) {
  aage[i] = (aage[i] + 1) % 10000;
}
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
    {
      ...
      for(p1 = ptable.proc; p1 < &ptable.proc[NPROC]; p1++) 
      {
        ...
        else if(selectp->ctime > p1->ctime) 
        {
          selectp = p1;
        } 
      }
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
    {
      ...
      for(p1 = ptable.proc; p1 < &ptable.proc[NPROC]; p1++) 
      {
        ...
        if(selectp->priority > p1->priority) 
        {
          selectp = p1;
        }
      }
...
start:for(i = 0; i < 4; i++) {
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
    {
      ...
      if(p->state == RUNNABLE && p->level == 4 )
       {
        if( ticks - p->last_run > 1000)
        push_back(p->pid, 3);
        
      }
      else if(p->state == RUNNABLE && ticks - p->last_run > 1000 )
       {
        if(p->level >= 1 && p->level <= 3)
        {
        i = aage[p->level];
        while(i < piche[p->level])
         {
          if(kataar[p->level][i] == p->pid) 
          {
            for(j = i+1; j < piche[p->level]; j++){
              kataar[p->level][j-1] = kataar[p->level][j];
            }
